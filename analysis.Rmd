...
```{r}
p <- ggplot(groups, aes(x=time, y=V3, colour=Type)) + geom_point(size=3) + theme_bw() + xlab("Linha do tempo") + ylab("Quantidade de Mensagens") + theme(axis.text=element_text(size=16), axis.title=element_text(size=16), legend.text = element_text(size=16), legend.title = element_text(size=16)) + guides(colour = guide_legend(override.aes = list(size=10)))
ggplotly(p)
```
